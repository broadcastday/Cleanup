...
## Deletes any OST file listed in a user profile.
$Users = get-childitem "C:\users" | select -ExpandProperty name
foreach ($User in $Users)
{
    get-childitem "C:\users\$user\AppData\Local\Microsoft\Outlook\*.ost" -Force -Verbose -ErrorAction SilentlyContinue | 
    Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete)) } |
    remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
}
## OST Files older than 2 weeks have been removed successfully!
...
## Microsoft Teams cache clearance - NEW 2023
Get-Childitem "C:\Users\*\AppData\Roaming\Microsoft\Teams\Service Worker\*" -Recurse -Force -ErrorAction SilentlyContinue |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue

## Box Log clearance - NEW 2023
Get-ChildItem "C:\Users\*\AppData\Local\Box\Box Edit\Logs\*" -Recurse -Force -ErrorAction SilentlyContinue |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
